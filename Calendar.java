...
public class Calendar {
	...
	public void addEvent(Appointment appt) {
		try {
			...
			pstmt.setInt(12, appt.getCategory());
		...
		try {
			...
			if(!rs.next()) {
				...
				do {
					...
					appt = new Appointment(rs.getDate(9).toLocalDate(), rs.getString(2), rs.getTime(8).toLocalTime(), rs.getString(11), rs.getInt(10), addr, rs.getInt(12));
					System.out.println();
	...
	public boolean displayEvent(int id) {
		try {
			...
			if(rs.next()) {
				...
				Appointment appt = new Appointment(rs.getDate(9).toLocalDate(), rs.getString(2), rs.getTime(8).toLocalTime(), rs.getString(11), rs.getInt(10), addr, rs.getInt(12));
				System.out.println("\n" + appt + "\n");
				return true;
			}
		} catch (SQLException e) {
		...
		try {
			...
			if (rs.next()) {
				...
				Appointment appt = new Appointment(rs.getDate(9).toLocalDate(), rs.getString(2),
						rs.getTime(8).toLocalTime(), rs.getString(11), rs.getInt(10), addr, rs.getInt(12));
